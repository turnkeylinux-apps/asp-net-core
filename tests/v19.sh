...
grep -Fxq 'channel=microsoft-debian-13-dotnet-10-and-stable-pomelo' "$update"
...
updater_result=$status; SDK candidate=$sdk_candidate; provider candidate=$provider_candidate
updater_channel=Microsoft Debian 13 .NET 10 LTS and stable NuGet Pomelo
integrity_evidence=Microsoft key $microsoft_key_fingerprint; NuGet lock SHA-256 $lock_sha256
EOF
